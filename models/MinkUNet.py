...
class MinkUNetBase(nn.Module):
    ...
    PLANES = (32, 64, 128, 256, 256, 128, 96, 96, 96)
    INIT_DIM = 32
    OUT_TENSOR_STRIDE = 1
    FPN_DEC_FEATS = 0
    ...
    def __init__(self, 
        ...
        self.conv0p1s1 = ME.MinkowskiConvolution(
            p_num_in_feats, self.inplanes, kernel_size=5, dimension=self.D
        )
        ...
        self.out_pred_1 = ME.MinkowskiConvolution(
            self.PLANES[8], 
            self.PLANES[8], 
            kernel_size=1, dimension=self.D)
        self.out_pred_norm_1 = ME.MinkowskiBatchNorm(
            self.PLANES[8])
        
        self.out_pred_2 = ME.MinkowskiConvolution(
            self.PLANES[8], 
            p_num_out_classes, 
            kernel_size=1, dimension=self.D)
        
        self.FPN_DEC_FEATS = self.PLANES[8]
    ...
    def forward(self, 
        ...
        in_field = ME.TensorField(
            ...
            coordinates=torch.cat(
                [p_out_pc.batch_ids_.unsqueeze(-1).int(), 
                 (p_out_pc.pts_/p_radius).int()], dim=1
            ),
            quantization_mode=ME.SparseTensorQuantizationMode.UNWEIGHTED_AVERAGE,
            minkowski_algorithm=ME.MinkowskiAlgorithm.SPEED_OPTIMIZED,
            device=p_in_feats.device,
        )
        x = in_field.sparse()
        ...
        out = self.final(out)
        hidden = out.slice(in_field).F

        out = self.out_pred_1(out)
        out = self.out_pred_norm_1(out)
        out = self.relu(out)
        
        out = self.out_pred_2(out).slice(in_field).F
...
class MinkUNet14A(MinkUNet14):
    PLANES = (32, 64, 128, 256, 128, 128, 96, 96, 96)

class MinkUNet14B(MinkUNet14):
    PLANES = (32, 64, 128, 256, 128, 128, 128, 128, 128)

class MinkUNet14C(MinkUNet14):
    PLANES = (32, 64, 128, 256, 192, 192, 128, 128, 128)

class MinkUNet14D(MinkUNet14):
    PLANES = (32, 64, 128, 256, 384, 384, 384, 384, 384)
...
class MinkUNet34B(MinkUNet34):
    PLANES = (32, 64, 128, 256, 256, 128, 64, 32, 32)
