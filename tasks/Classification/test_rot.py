...
def create_hierarchy(p_model_dict, p_pts, p_batch_ids, p_features, p_test_dict):
    with torch.no_grad():
        ...
        # compute local reference frames for new pc
...
############## TEST
def test(p_model_dict, p_models, p_data_loader, p_accum_logits, p_classes, p_test_dict):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf_file",
        default="confs/modelnet40_test_rot.yaml",
        help="Configuration file (default: confs/modelnet40_test_rot.yaml)",
    )
    # parser.add_argument(
    ...
    # Set the gpu id.
    GPU_ID = args.gpu
    model_paths = ["path to your saved model/model_epoch_500.pth"]  # args.saved_model]
    n_frames_testing = [2]
    ...
    for m_path in model_paths:
        for n_frames in n_frames_testing:
            ...
            exp_results_folder.mkdir(parents=True, exist_ok=False)
            ...
            if "lieconv" in m_path:
                ...
                test_dict["RefFrames"] = {
                    "pca": True,
                    "neigh_method": "knn",
                    "neigh_kwargs": {"neigh_k": 0},
                    "fixed_axis": False,
                    "n_frames": n_frames,
                }

                test_dict["RefFrames"]["neigh_kwargs"]["neigh_k"] = 16
            ...
            for cur_epoch in range(test_dict["num_epochs"]):
                ...
                # Test.
                test(model_dict, models, testdl, accum_logits, classes, test_dict)
