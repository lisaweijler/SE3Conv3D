...
def create_hierarchy(p_model_dict, p_pts, p_batch_ids, p_features):
    with torch.no_grad():
        ...
        if "mix_n_frames" in p_model_dict["RefFrames"]:
            mix_f_dict = p_model_dict["RefFrames"]["mix_n_frames"]
            frames_list = [4, 2, 1]
            prob_list = [mix_f_dict[f] for f in frames_list]
            n_frames_chosen = np.random.choice(frames_list, size=1, p=prob_list)[0]

            p_model_dict["RefFrames"]["n_frames"] = n_frames_chosen

        fake_features = torch.ones(
            (new_features.shape[0] * p_model_dict["RefFrames"]["n_frames"], 1),
        ...
        return hierarchy, fake_features, levels_radii
...
def pre_process(p_model_dict, p_model, p_data_loader):
    with torch.no_grad():
        ...
        for cur_iter, cur_batch in enumerate(p_data_loader):
            ...
            p_model(hierarchy, features, lev_radii)
...
def train(
    ...
    for cur_iter, cur_batch in enumerate(p_data_loader):
        ...
        # repeat features for first layer:

        pred = p_model(hierarchy, features, lev_radii)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf_file",
        default="confs/modelnet40_pca_2F.yaml",
        help="Configuration file (default: confs/modelnet40_pca_2F.yaml)",
    )
    parser.add_argument(
        "--data_folder",
    ...
    pre_process(model_dict_train, model, traindl)
    ...
    for cur_epoch in range(train_dict["num_epochs"]):
        ...
        if cur_epoch % train_dict["val_freq"] == 0:
            ...
            start_epoch_time = current_milli_time()
            val_acc, val_loss = validation(model_dict_test, model, testdl, loss_fn)
            end_epoch_time = current_milli_time()
